...
## Using GraphSwarm on Any Project

GraphSwarm works on any Rust, Python, JavaScript, or TypeScript codebase -not just its own source.

### Step 1 -Index the target project

```bash
# Windows
target\release\graphswarm.exe index "D:\path\to\project"

# Linux / Mac
./target/release/graphswarm index /path/to/project
```

The call graph is written to `<project>/.graphswarm/db/` inside the target project directory.

### Step 2 -Query that project

```bash
# Windows
target\release\graphswarm.exe query "authentication" --index "D:\path\to\project\.graphswarm\db"

# Linux / Mac
./target/release/graphswarm query "authentication" --index /path/to/project/.graphswarm/db
```

### Step 3 -Export its visual graph

```bash
# Windows
target\release\graphswarm.exe export "D:\path\to\project"

# Linux / Mac
./target/release/graphswarm export /path/to/project
```

Output is written to `graphswarm-out/`. Open `graphswarm-out\graph.html` in any browser -no internet connection required.

### Step 4 -Install the skill file into that project

```bash
# Windows
target\release\graphswarm.exe install --project "D:\path\to\project"

# Linux / Mac
./target/release/graphswarm install --project /path/to/project
```

Creates `.claude/skills/graphswarm/SKILL.md`, `.cursorrules`, and `AGENTS.md` inside the target project directory.

### Step 5 -Start the MCP server watching that project
...
The AI agent reads `SKILL.md` and routes codebase questions through GraphSwarm automatically.

### Add to PATH (run once)

Adding the GraphSwarm binary to your PATH means you can run it from any directory without specifying the full binary path each time.

**Windows** (run as Administrator):

```bat
setx PATH "%PATH%;D:\USC SUBJECTS\USC SPRING 2026\GraphSwarm\target\release"
```
...
After adding to PATH, from any project directory:

```bash
graphswarm index .
graphswarm query "storage"
